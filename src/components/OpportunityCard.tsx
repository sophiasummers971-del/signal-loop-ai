...
import { Target, Users, DollarSign, ChevronDown, ChevronUp, Rocket, CheckCircle2 } from "lucide-react";
...
const OpportunityCard = ({ opportunity }: { opportunity: Opportunity }) => {
  ...
  const matchColor = opportunity.match >= 90
    ? "text-success"
  ...
  return (
    <div className="bg-card rounded-xl border p-6 hover:shadow-lg transition-all duration-300 group">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={`font-display font-bold text-lg ${matchColor}`}>
          {opportunity.match}%
        </div>
      </div>
      ...
      <button
        ...
        className="text-sm font-medium text-accent flex items-center gap-1 mb-3"
      >
        ...
        {expanded && (
          ...
          >
            <ol className="space-y-2 mb-4">
              {opportunity.mvpSteps.map((step, i) => (
