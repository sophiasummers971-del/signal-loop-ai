...
import { ArrowRight, ArrowLeft, Zap, CheckCircle2, User, Briefcase, Clock, Target, Palette } from "lucide-react";
...
const Onboarding = () => {
  ...
  const handleFinish = () => {
    // For now just navigate to dashboard with sample data
    navigate("/dashboard");
  };
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {interestOptions.map((interest) => (
                    ...
                    >
                      {formData.interests.includes(interest) && <CheckCircle2 className="w-4 h-4 inline mr-1" />}
                      {interest}
                    </button>
                  ))}
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm font-medium text-foreground mb-1 block">
                        <Clock className="w-4 h-4 inline mr-1" /> Hours/week
                      </label>
                      <select
                        value={formData.hoursPerWeek}
                        onChange={(e) => setFormData({ ...formData, hoursPerWeek: e.target.value })}
                        className="w-full h-12 rounded-lg border bg-card px-3 text-sm text-foreground"
                      >
                    ...
                    <div>
                      <label className="text-sm font-medium text-foreground mb-1 block">
                        <Target className="w-4 h-4 inline mr-1" /> Monthly goal
                      </label>
                      <select
        ...
        <div className="flex justify-between mt-10">
          ...
          {step < totalSteps - 1 ? (
            ...
            <Button
              ...
              className="gap-1"
            >
              <Zap className="w-4 h-4" /> Generate My Opportunities
            </Button>
          )}
        </div>
      </div>
